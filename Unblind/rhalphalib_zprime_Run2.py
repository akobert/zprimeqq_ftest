...
def test_rhalphabet(tmpdir, sig, throwPoisson=False):
    ...
    tqq_xs = rl.NuisanceParameter('tqq_xs_Run2', 'lnN', 1., 0, 10)
    WG_xs = rl.NuisanceParameter('WG_xs_Run2', 'lnN', 1., 0, 10)
    ZG_xs = rl.NuisanceParameter('ZG_xs_Run2', 'lnN', 1., 0, 10)

    # Systematics
    sys_lumi = rl.NuisanceParameter('CMS_lumi_13TeV_Run2', 'lnN')
    sys_phoID = rl.NuisanceParameter('CMS_phoID_13TeV_Run2', 'lnN')
    sys_trig = rl.NuisanceParameter('CMS_trig_13TeV_Run2', 'lnN') #Trigger Systematic

    sys_dict = {}
    exp_systs = {} # dictionary of tag dictionaries
    for y in years:
        sys_dict['jes_'+y] = rl.NuisanceParameter('CMS_scale_j_{}'.format(y), 'lnN')
        sys_dict['jer_'+y] = rl.NuisanceParameter('CMS_res_j_{}'.format(y), 'lnN')
    
        sys_dict['pu_'+y] = rl.NuisanceParameter('CMS_PU_{}'.format(y), 'lnN')

        sys_dict[args.tagger+'_'+y] = rl.NuisanceParameter('CMS_'+str(args.tagger)+'eff_{}'.format(y), 'lnN') # N2DDT or ParticleNet Systematic

        sys_dict['smear_'+y] = rl.NuisanceParameter('CMS_smear_{}'.format(y), 'shape')
        sys_dict['scale_'+y] = rl.NuisanceParameter('CMS_scale_{}'.format(y), 'shape')

        # Systematic Tags
        exp_systs[y] = ['pu_'+y, 'jes_'+y, 'jer_'+y, args.tagger+'_'+y, 'smear_'+y, 'scale_'+y]
    

    # define bins    
    ptbins = {} # Dictonary of bins for each year
    ptbins['2016'] = np.array([200, 225, 245, 275, 325, 700])
    ptbins['2017'] = np.array([220, 230, 255, 290, 360, 700])
    ptbins['2018'] = np.array([120, 170, 220, 300, 400, 700])
    ...
    while fitfailed_GJ < 5:
        ...
        for y in ['2018']:
            ...
            ptpts, msdpts = np.meshgrid(ptbins[y][:-1] + 0.5 * np.diff(ptbins[y]), msdbins[:-1] + 0.5 * np.diff(msdbins), indexing='ij')
            print(ptpts)
            print(msdpts)
            rhopts = 2*np.log(msdpts/ptpts)
            print(rhopts)
            ptscaled = (ptpts - ptbins[y][0]) / (ptbins[y][-1] - ptbins[y][0])
            ...
            for ptbin in range(npt):
                failCh = rl.Channel('ptbin%d%s' % (ptbin, 'fail')+y)
                passCh = rl.Channel('ptbin%d%s' % (ptbin, 'pass')+y)
...
#                passCh = rl.Channel('ptbin%d%s' % (ptbin, 'pass'))
                ...
                failTempl = get_template('GJ', 0, ptbin+2, y, obs=msd, syst='nominal') 
                passTempl = get_template('GJ', 1, ptbin+2, y, obs=msd, syst='nominal') 
            ...
            for ptbin in range(npt):
                failCh = GJmodel['ptbin%dfail' % (ptbin)+y]
                passCh = GJmodel['ptbin%dpass' % (ptbin)+y]
...
#                passCh = GJmodel['ptbin%dpass' % ptbin]
                ...
                GJparams = np.array([rl.IndependentParameter('GJparam_ptbin%d_msdbin%d_%s' % (ptbin, i, y), 0) for i in range(msd.nbins)])
#                GJparams = np.array([rl.IndependentParameter('GJparam_ptbin%d_msdbin%d' % (ptbin, i), 0) for i in range(msd.nbins)])
                ...
                fail_GJ = rl.ParametericSample('ptbin%dfail%s_GJ' % (ptbin,y), rl.Sample.BACKGROUND, msd, scaledparams[0])
#                fail_GJ = rl.ParametericSample('ptbin%dfail_GJ' % ptbin, rl.Sample.BACKGROUND, msd, scaledparams[0])
                failCh.addSample(fail_GJ)
                pass_GJ = rl.TransferFactorSample('ptbin%dpass%s_GJ' % (ptbin,y), rl.Sample.BACKGROUND, tf_MCtempl_params[ptbin, :], fail_GJ)
    ...
    for y in years:
        ...
        for ptbin in range(npt):
            for region in ['pass', 'fail']:
                ...
                ch = rl.Channel('ptbin%d%s' % (ptbin, region)+y)
#                ch = rl.Channel('ptbin%d%s' % (ptbin, region))
                log.debug(f"Initializing model region: "+'ptbin%d%s' % (ptbin, region)+y)   
                model.addChannel(ch)
                ...
                for sName in samps:
                    ...
                    templates[sName] = get_template2(sName, isPass, ptbin+2, y, obs=msd, syst='nominal')
                    ...
                    def smorph(templ):      
                        ...
                        if MORPHNOMINAL and sName not in ['GJ']:
                            return MorphHistW2(templ).get(shift=SF[y]['shift_SF']/smass('Wjets') * smass(sName),
                                                          smear=SF[y]['smear_SF']
                                                      )
                        else:
                            return templ
...
#                    print(sName+" "+region+" Expectation: "+str(sample.getExpectation(nominal=True).sum()))
                    ...
                    if do_systematics:
                        ...
                        other_systs = ['pu', 'jes', 'jer']
                        for sys in other_systs:
                            syst_up = get_template(sName, isPass, ptbin+2, y, obs=msd, syst=sys+'Up')[0]
                            syst_do = get_template(sName, isPass, ptbin+2, y, obs=msd, syst=sys+'Down')[0]

                            eff_up = shape_to_num(syst_up, nominal)
                            eff_do = shape_to_num(syst_do, nominal)

                            sample.setParamEffect(sys_dict[sys+'_'+y], eff_up, eff_do)

                    # Scale and Smear
                    mtempl = AffineMorphTemplate(templ)
                    if sName not in ['QCD']:
                        # shift
                        realshift = SF[y]['shift_SF_ERR']/smass('Wjets') * smass(sName)
                        ...
                        if badtemp_ma(_up[0]) or badtemp_ma(_down[0]):
                            ...
                            sample.setParamEffect(sys_dict['scale_'+y], _up, _down, scale=1)
...
#                            ofile_morph.WriteObject(num_to_hist(_down[0], sName+" ScaleDown Weights ptBin "+str(ptbin)),sName+"_scaleDown_weights_"+region+str(ptbin))
                        ...
                        # smear
                        _up = mtempl.get(smear=1 + SF[y]['smear_SF_ERR'])
                        _down = mtempl.get(smear=1 - SF[y]['smear_SF_ERR'])
                        if badtemp_ma(_up[0]) or badtemp_ma(_down[0]):
                            print("Skipping sample {}, scale systematic would be empty".format(sName))
                        else:
                            sample.setParamEffect(sys_dict['smear_'+y], _up, _down)
...
#                            ofile_morph.WriteObject(num_to_hist(_down[0], sName+" SmearDown Weights ptBin "+str(ptbin)),sName+"_smearDown_weights_"+region+str(ptbin))
                    ...
                    # Tagger SF
                    sample.scale(SF[y][args.tagger+'_SF'])        # N2DDT Scale Factor Applied Here
                    if do_systematics:
                        effect = 1.0 + SF[y][args.tagger+'_SF_ERR'] / SF[y][args.tagger+'_SF'] # N2DDT Systematic Applied Here
                        sample.setParamEffect(sys_dict[args.tagger+'_'+y],effect)

                    ch.addSample(sample)

                data_obs = get_template2('Data', isPass, ptbin+2, y, obs=msd, syst='nominal')
...
#            passCh = model['ptbin%dpass' % ptbin]
            failCh = model['ptbin%dfail' % (ptbin)+y]
            passCh = model['ptbin%dpass' % (ptbin)+y]
            Dataparams = np.array([rl.IndependentParameter('Dataparam_ptbin%d_msdbin%d_%s' % (ptbin, i, y), 0) for i in range(msd.nbins)])
...
#                raise ValueError('initial_Data negative for some bins..', initial_Data)
            ...
            fail_NonRes = rl.ParametericSample('ptbin%dfail%s_NonRes' % (ptbin, y), rl.Sample.BACKGROUND, msd, scaledparams)
#            fail_NonRes = rl.ParametericSample('ptbin%dfail_NonRes' % ptbin, rl.Sample.BACKGROUND, msd, scaledparams)
            ...
            pass_NonRes = rl.TransferFactorSample('ptbin%dpass%s_NonRes' % (ptbin, y), rl.Sample.BACKGROUND, tf_params[ptbin, :], fail_NonRes)
